...
(defun distancia-vitoria (tabuleiro jogador &optional (x 1) (y 1))
  ...
  (cond ((> x 7) 0)
        ((> y 7) (distancia-vitoria tabuleiro jogador (1+ x) 1))
        (t (let* ((cel (celula x y tabuleiro)))
              (cond ((or (null cel) (/= cel jogador)) (distancia-vitoria tabuleiro jogador x (1+ y)))
                    (t (let ((dist-atual (cond 
                                          ...
                                          ((= jogador *jogador2*)
                                           (cond 
                                             ...
                                             ((< y 4) (+ (abs (- x 2)) (abs (- y 3))))))
                                          (t 1000))))
                          (+ dist-atual (distancia-vitoria tabuleiro jogador x (1+ y)))
                        )
                    )
              )
            )
