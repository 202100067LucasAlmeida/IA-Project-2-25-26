...
;;; Fun  es de Visualiza  o Formatada

(defun mostrar-tabuleiro (tabuleiro)
  "Mostra o tabuleiro com formata  o alinhada eleg vel"
  (format t "~%    1    2    3    4    5    6    7~%")
  (format t "  +----+----+----+----+----+----+----+~%")
  (mostrar-linhas tabuleiro 1)
  (format t "  +----+----+----+----+----+----+----+~%")
)

(defun mostrar-linhas (tabuleiro n)
  "Mostra recursivamente cada linha do tabuleiro formatada"
  (cond ((null tabuleiro) nil)
        (t (format t "~A |" n)
           (mostrar-celulas (car tabuleiro))
           (format t "~%  +----+----+----+----+----+----+----+~%")
           (mostrar-linhas (cdr tabuleiro) (1+ n))))
)

(defun mostrar-celulas (linha)
  "Mostra recursivamente cada c lula de uma linha formatada"
  (cond ((null linha) nil)
        (t (let ((cel (car linha)))
             (format t "~A|" (formatar-celula cel))
             (mostrar-celulas (cdr linha)))))
)

(defun formatar-celula (valor)
  "Formata o valor da c lula para exibi  o alinhada"
  (cond ((null valor) "    ")
        ((= valor 0) "  . ")
        ((= valor *jogador1*) " J1 ")
        ((= valor *jogador2*) " J2 ")
...
(defun mostrar-estado-jogo (tabuleiro jogador)
  "Mostra o tabuleiro e informa  es do estado atual"
  (format t "~%~%========================================~%")
  (format t "         ESTADO DO JOGO~%")
  (format t "========================================~%")
  (format t "~%Jogador atual: ~A~%" 
          (cond ((= jogador *jogador1*) "Jogador 1 (J1 - topo)")
                ((= jogador *jogador2*) "Jogador 2 (J2 - baixo)")
                (t "Desconhecido")))
  (mostrar-tabuleiro tabuleiro)
  (format t "~%Pe~\Aes J1: ~A  |  Pe~\Aes J2: ~A~%" 
          #\c (quantidade-peao tabuleiro *jogador1*)
          #\c (quantidade-peao tabuleiro *jogador2*))
  (format t "========================================~%~%")
)

(defun mostrar-resultado (tabuleiro vencedor)
  "Mostra o tabuleiro final e o vencedor"
  (format t "~%~%========================================~%")
  (format t "         FIM DE JOGO!~%")
  (format t "========================================~%")
  (mostrar-tabuleiro tabuleiro)
  (format t "~%")
  (format t "*** VENCEDOR: ~A ***~%"
          (cond ((= vencedor *jogador1*) "Jogador 1 (J1)")
                ((= vencedor *jogador2*) "Jogador 2 (J2)")
                (t "Empate")))
  (format t "========================================~%~%")
)
